...
const insertCategory = (data) => {
  const { id_category, name_category, image_category } = data;
  return Pool.query(
    `INSERT INTO category(id_category,name_category,image_category) VALUES(${id_category},'${name_category}','${image_category}')`
  );
};
...
const updateCategory = (data) => {
  const { id_category, name_category, image_category } = data;
  return Pool.query(
    `UPDATE category SET name_category='${name_category}',image_category='${image_category}' WHERE id_category=${id_category}`
  );
};
...
const searchCategory = (name_category) => {
  return Pool.query(`SELECT * FROM category WHERE name_category ILIKE '%${name_category}%'`);
};
