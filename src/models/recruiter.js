...
const createRecruiterVerification = (
  ...
  return Pool.query(
    `insert into users_verification ( id , recruiter_id , token ) values ( '${recruiter_verification_id}' , '${recruiter_id}' , '${token}' )`
  );
};

const checkRecruiterVerification = (queryUsersId, queryToken) => {
  return Pool.query(
    `select * from users_verification where recruiter_id='${queryUsersId}' and token = '${queryToken}' `
  );
};
...
const deleteRecruiterVerification = (queryUsersId, queryToken) => {
  return Pool.query(
    `delete from users_verification  where recruiter_id='${queryUsersId}' and token = '${queryToken}' `
  );
};
